...
const Profile = () => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try{
      ...
      const response = await changeAvatar(url, token);
      if ("Profile picture updated successfully") {
                  ...
                  <legend className='w-full'>
                    ...
                    <div className='flex flex-wrap gap-5 items-center px-10'>
                      ...
                      <form className='flex flex-col movil:w-full tablet:w-[48%] pt-5 items-left'>
                        ...
                        <input className='bg-transparent border-b border-r text-center w-full' type={showPassword ? "text" : "password"} id='password' name='password' value={password} onChange={(e) => setPassword(e.target.value)} />
                        <button className='relative left-[90%] bottom-6 w-5' onClick={(e) => { e.preventDefault(); setShowPassword(!showPassword); }}>
                          {showPassword ? hide : show}
                        </button>
                        <button type='submit' onClick={handlePasswordChange}> change</button>
