...
export const Accounts = ({ id, number, balance, creationDate }) => {
  ...
  return (
    <article className='min-w-[48%] max-w-full w-auto'>
    ...
    <div key={id} className='flex flex-col w-full text-white gap-1 pb-3'>
            <h2 className='font-bold flex w-full text-xl'>Account Number:  {number} <p className='creationDate'>Creation date: {creationDate}</p></h2>
            <h3 className='font-bold self-end px-4 py-2 text-right border-b border-l text-lg'>My Balance:  {parseFloat(balance).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</h3>
